...
const dependencyGraphBuilder_1 = require("./services/dependencyGraphBuilder");
...
async function main() {
    ...
    if (cmd === 'build-graph') {
        const repositoryId = requireArg('--repo-id');
        const revisionSha = getArgValue('--revision-sha');
        await (0, dependencyGraphBuilder_1.buildDependencyGraph)({ repositoryId, revisionSha });
        return;
    }
    if (cmd === 'index-search') {
        const repositoryId = requireArg('--repo-id');
        const revisionSha = getArgValue('--revision-sha');
        await (0, searchIndex_1.indexRepositorySearch)({ repositoryId, revisionSha });
        return;
    }
    if (cmd === 'review-pr') {
        const repositoryId = requireArg('--repo-id');
    ...
    if (cmd === 'ingest-history') {
        ...
        const result = await (0, historyIngest_1.ingestRepositoryHistory)({
            repositoryId,
            repoPath,
            rebuild,
            maxCount
    ...
    console.error('Usage: node api/dist/cli.js <sync-repo|build-graph|index-search|review-pr|ingest-history> --repo-id <repositoryId> [--revision-sha <sha>] [sync-repo flags: --path <repoPath> --repo-name <name> --owner <owner> --concurrency <n>] [review-pr flags: --pull-number <n> --force] [ingest-history flags: --path <repoPath> --rebuild --max-count <n>]');
    process.exit(1);
}
main().catch((err) => {
    console.error(err);
